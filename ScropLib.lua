--[[
    ...
    USAGE
        local Scrop = loadstring(game:HttpGet("YOUR_RAW_URL/ScropLib.lua"))()
    ...
    See Scrop.lua for a full placeholder hub.
]]
...
Library.ThemeOrder = { "Silver Surfer", "Power Cosmic", "Zenn-La", "Deep Space" }
...
local function Fire(cb, ...)
    ...
    if not ok then warn("[Scrop UI] Callback error: " .. tostring(err)) end
end
...
--[[
    opts:
        Title          string   – logo text in the sidebar                      ("SCROP")
...
function Library:CreateWindow(opts)
    ...
    self.Title        = opts.Title or "SCROP"
    self.Flags        = {}
    self._setters     = {}
    self._conns       = {}
    self._binds       = {}
    ...
    self.ConfigFolder = opts.ConfigFolder or ("Scrop_" .. self.Title:gsub("[^%w_]", ""))
    ...
    if type(opts.Theme) == "table" then
        ...
        self.ThemeName = Library.Themes[opts.Theme or ""] and opts.Theme or "Silver Surfer"
        preset = Library.Themes[self.ThemeName]
    end
    self.Theme = BuildTheme(preset)

    -- Replace an existing instance of the same window
    local guiName = "ScropUI_" .. self.Title:gsub("[^%w_]", "")
    local old = shared.ScropUIWindows[guiName]
    if old then pcall(function() old:Destroy(true) end) end
    shared.ScropUIWindows[guiName] = self
    self._id = guiName
    ...
    if opts.Subtitle then
        Make("TextLabel", {
            ...
            Text = opts.Subtitle, TextColor3 = self.Theme.TextDim, Font = Enum.Font.Gotham, TextSize = 11,
        })
    end
    ...
    Make("TextLabel", { Parent = searchBg, Size = UDim2.new(0, 25, 1, 0), Position = UDim2.new(0, 5, 0, 0), BackgroundTransparency = 1, Text = "🔍", Font = Enum.Font.Gotham, TextSize = 11, TextColor3 = self.Theme.TextDim })
    local searchBox = Make("TextBox", {
        Parent = searchBg, Size = UDim2.new(1, -30, 1, 0), Position = UDim2.new(0, 30, 0, 0), BackgroundTransparency = 1,
    ...
    if opts.ToggleWidget ~= false then
        ...
        Make("TextLabel", {
            Parent = w, Size = UDim2.new(1, -10, 1, 0), Position = UDim2.new(0, 5, 0, 0), BackgroundTransparency = 1,
            Text = (opts.WidgetText or self.Title):upper(), TextColor3 = Color3.new(1, 1, 1),
            Font = Enum.Font.GothamBlack, TextSize = 14, TextTruncate = Enum.TextTruncate.AtEnd,
        })
    ...
    -- ── Gradient breathing loop ──
    do
        local t, acc = 0, 0
        self:_connect(RunService.RenderStepped, function(dt)
            t = t + dt * 1.5
            acc = acc + dt
            local offset = Vector2.new(math.sin(t) * 1.2, 0)
            local list = self._breathing
            for i = #list, 1, -1 do
                local g = list[i]
                if g.Parent then g.Offset = offset
                elseif acc > 2 then table.remove(list, i) end
            end
            if acc > 2 then acc = 0 end
        end)
    end
...
function Window:Destroy(instant)
    ...
    if shared.ScropUIWindows[self._id] == self then shared.ScropUIWindows[self._id] = nil end
...
function Window:Notify(title, desc, duration, color)
    ...
    self:_play("Click")
    Tween(notif, { Position = UDim2.new(0, 0, 0, 0) }, 0.4, Enum.EasingStyle.Quint, Enum.EasingDirection.Out)
...
function Window:SetWatermark(text)
    if not self._wm then
        ...
        Make("UIPadding", { Parent = self._wm, PaddingLeft = UDim.new(0, 12), PaddingRight = UDim.new(0, 12) })
...
function Tab:CreateSection(title, expanded)
    ...
    local arrow = Make("TextLabel", {
        ...
        Text = expanded and "▼" or "▶", Font = Enum.Font.GothamBold, TextSize = 12,
    })
...
function Section:AddSlider(name, o)
    ...
    track.InputBegan:Connect(function(input) if IsPress(input) then dragging = true; fromInput(input) end end)
    win:_connect(UserInputService.InputEnded, function(input) if IsPress(input) then dragging = false end end)
...
function Section:AddDropdown(name, o)
    ...
    local btn = Make("TextButton", {
        ...
        TextColor3 = win.Theme.TextWhite, Font = Enum.Font.Gotham, TextSize = 12, AutoButtonColor = false,
    })
    win:_bind(function(t) btn.BackgroundColor3 = t.ToggleOff end)
    Corner(btn, 6)
    ...
    local function updateText()
        btn.Text = (current ~= nil and tostring(current) or "Select...") .. "   " .. (isOpen and "▲" or "▼")
    end
    ...
    local function build()
        ...
        for i, opt in ipairs(options) do
            local ob = Make("TextButton", {
                Parent = list, Size = UDim2.new(1, 0, 0, OPT_H), BackgroundTransparency = 1, LayoutOrder = i, Text = tostring(opt),
                TextColor3 = win.Theme.TextDim, Font = Enum.Font.Gotham, TextSize = 12, AutoButtonColor = false,
            })
            ob.MouseEnter:Connect(function() Tween(ob, { TextColor3 = win.Theme.TextWhite, BackgroundTransparency = 0.8 }, 0.15) end)
            ob.MouseLeave:Connect(function() Tween(ob, { TextColor3 = win.Theme.TextDim, BackgroundTransparency = 1 }, 0.15) end)
